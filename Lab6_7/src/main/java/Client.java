...
public class Client {
    ...
    public static void main(String[] args) {
        try {
            ...
            server.registerPlayer(player1);
            Player player2 = new Player("gracz2");
            server.registerPlayer(player2);
            server.challengePlayer(player1.token, player2.getName());
//            Menu();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void Lobby() throws RemoteException, InterruptedException {
        ...
        while(true){
            ...
            System.out.println("2. Wyzwanie gracza");
            System.out.println("3. Wyświetl stan gry");
            System.out.println("4. Wykonaj ruch");
            ...
            switch (choice) {
                ...
                case 2: // Wyzwanie gracza
                    ...
                    server.challengePlayer(token, opponentName);
                    System.out.println("Oczekiwanie na przeciwnika...");
                    while (true){
                        if(server.checkForGame(token)){
                            System.out.println("Nowa gra rozpoczęta! Twój przeciwnik to: " + server.getOpponentName(player));
                            break;
                            //przejscie do funkcji processGame
                        } else {
                            System.out.println("Oczekiwanie na przeciwnika...");
                        }
                        Thread.sleep(3000);
                    }
                case 4: // Wyświetl stan gry
                    System.out.println("Stan gry:");
                    System.out.println(server.getGameState(token));
                    break;
