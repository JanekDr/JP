...
public class ProductDialog extends JDialog {
    ...
    private void onOK() {
        ...
        if (name.isEmpty() || carbsText.isEmpty() || proteinText.isEmpty() || fatText.isEmpty() || category.isEmpty() || quantityText.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Proszę wypełnić wszystkie pola.", "Brak Danych", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        try {
            ...
            if (carbs < 0 || protein < 0 || fat < 0 || quantity < 0) {
                JOptionPane.showMessageDialog(this, "Wartości muszą być dodatnie.", "Błąd Wprowadzania", JOptionPane.ERROR_MESSAGE);
                return;
            }

            if (product == null) {
                // Tworzenie nowego produktu bez ID (menedżer przypisze ID)
                product = new Product(0, name, carbs, protein, fat, category, quantity);
            } else {
            ...
            JOptionPane.showMessageDialog(this, "Proszę wprowadzić prawidłowe liczby.", "Błąd Wprowadzania", JOptionPane.ERROR_MESSAGE);
        }
    }
