...
public class DietBuilderGUI extends JFrame {
    ...
    private void deleteProduct(JList<Product> productList) {
        Product selectedProduct = productList.getSelectedValue();
        if (selectedProduct != null) {
            int confirm = JOptionPane.showConfirmDialog(this, "Czy na pewno chcesz usunac wybrany produkt?", "Potwierdzenie Usuniecia", JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                boolean success = productManager.removeProduct(selectedProduct.getId());
                if (success) {
    ...
    private void editMeal(JList<Meal> mealList) {
        ...
        if (selectedMeal != null) {
            EditMealDialog dialog = new EditMealDialog(this, "Edytuj Posiłek", selectedMeal, productManager);
            dialog.setVisible(true);
    ...
    public List<Meal> getAllMeals() {
        ...
        for (int i = 0; i < mealListModel.size(); i++) {
            meals.add(mealListModel.getElementAt(i));
        }
        return meals;
    }
